...
if [ "$#" -ne 2 ]; then
  echo "please input orderer name & secret"
  exit 1
fi
...
sed -i "s/MSP__NAME/${ORDERER__NAME}/g" ${DOCKER_PATH}/fabric-ca-client-config.yaml
...
sed -i "s/MSP__NAME/${ORDERER__NAME}/g" ${DOCKER_PATH}/enroll.sh
